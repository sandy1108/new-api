...
本文档记录已确认的访问层边界和一期 Web 集成方案。方案已在开发 Worktree 实现，并于 2026-08-31 完成隔离 API/UI 回归；后续只需按项目发布门槛进行 Git 收尾和发布审批。
...
- 已在 `new-api-development` 的 `upgrade/upstream-main-20260831` 分支实现 `web/src/features/usage-summary/`、认证路由 `/usage-summary`、侧边栏入口和多语言文案；前端通过共享 `api` 实例调用聚合接口。
- Node.js 24.18.0 下 `npx vitest run` 的 65 个测试文件、434 个测试全部通过；`npx tsgo -b`、本次涉及文件的 `oxlint` 和 `oxfmt --check` 通过。全量格式检查仍受 25 个官方既有文件问题影响，未修改无关文件。
- 使用 `new-api:dev-20260831-02-g812702ba` 在临时隔离栈完成真实浏览器回归：管理员页面默认调用 `/api/log/usage-summary`，切换“仅自己”调用 `/api/log/self/usage-summary`；今天、昨天、本周、上周、本月、上月、本季度七个范围均各产生一次聚合请求，刷新再次只产生一次当前范围请求，未调用旧 `/api/log/` 分页接口。
- 空数据提示、管理员范围切换和刷新控件均正常；`/usage-logs/common` 仍可打开并继续使用原分页接口。临时容器、网络和数据卷已清理，生产容器和数据未触碰。
- 宿主机完整 Rsbuild 仍受现有 `node_modules` 缺少 `@lobehub/ui`、`antd` 阻塞；未擅自改依赖。仓库原生 Dockerfile 的完整镜像构建已通过，因此该阻塞不影响本轮隔离镜像验收。

## 三级明细 UI 修正（2026-08-31）

用户审核视觉预览后确认，原先页面中“按 Token 汇总”和渠道卡片的层级表达不够准确。本次按 Chrome 插件的实际交互修正为：

```text
按 API 令牌汇总 → 选择 API 令牌 → 渠道汇总 → 选择渠道 → 渠道内模型汇总
```

修正内容：

- 页面一级标题改为“API Token Summary”，表格列只表达 API 令牌、用户、请求、总 Token 和 quota。
- Token 详情不再展示跨渠道混合的模型合计；默认选择该 Token 总 Token 最高的渠道。
- 渠道使用可访问按钮，点击只更新本地 `selectedChannelKey`，下方模型表只渲染当前渠道的模型。
- 汇总卡片和明细数值采用 `formatUsageCompactNumber` 显示 `千/万/亿`，并在 `title` 中保留完整千分位值。
- 交互和状态只使用 Web 自身的 React state 与共享 API 查询，不引入 Chrome 插件的认证、Cookie 或缓存实现。

本次实现涉及 `web/src/features/usage-summary/` 的组件、选择器、格式化函数和测试，以及新增的中文/英文/繁体中文文案键；后端接口、数据库、权限、缓存和生产运行边界均保持不变。
