...
describe('createUsageSummaryRange', () => {
  ...
  test('yesterday covers the previous complete local calendar day', () => {
    ...
    expect(range.startTimestamp).toBe(timestamp('2026-08-04T00:00:00+08:00'))
    expect(range.endTimestamp).toBe(timestamp('2026-08-04T23:59:59+08:00'))
  })

  test('this week starts on Monday and ends at now', () => {
    const range = createUsageSummaryRange('week', NOW)

    expect(range.startTimestamp).toBe(timestamp('2026-08-03T00:00:00+08:00'))
    expect(range.endTimestamp).toBe(timestamp('2026-08-05T14:32:18+08:00'))
  })

  test('last week covers the previous complete Monday-to-Sunday week', () => {
    const range = createUsageSummaryRange('last-week', NOW)

    expect(range.startTimestamp).toBe(timestamp('2026-07-27T00:00:00+08:00'))
    expect(range.endTimestamp).toBe(timestamp('2026-08-02T23:59:59+08:00'))
  })

  test('this month starts on the first local day and ends at now', () => {
    const range = createUsageSummaryRange('month', NOW)

    expect(range.startTimestamp).toBe(timestamp('2026-08-01T00:00:00+08:00'))
    expect(range.endTimestamp).toBe(timestamp('2026-08-05T14:32:18+08:00'))
  })
  ...
  test('last month covers the previous complete local calendar month', () => {
    ...
    expect(range.startTimestamp).toBe(timestamp('2026-07-01T00:00:00+08:00'))
    expect(range.endTimestamp).toBe(timestamp('2026-07-31T23:59:59+08:00'))
  })
